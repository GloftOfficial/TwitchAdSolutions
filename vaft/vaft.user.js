...
(function() {
    ...
    function declareOptions(scope) {
        ...
        scope.FixPlayerBufferingInsideAds = false;
    ...
    async function processM3U8(url, textStr, realFetch) {
        ...
        if (haveAdTags) {
            ...
            for (var playerTypeIndex = startIndex; !backupM3u8 && playerTypeIndex < BackupPlayerTypes.length; playerTypeIndex++) {
				...
				// Handle proxy player type
				if (playerType === 'proxy') {
					try {
						var proxyUrl = String(atob(atob('YUhSMGNITTZMeTloWkdKc2IyTnJMbWRzYjJaMExtMWxMejlqYUdGdVBRPT0='))) + streamInfo.ChannelName;
						var encodingsM3u8Response = await realFetch(proxyUrl, {method: 'GET'});
